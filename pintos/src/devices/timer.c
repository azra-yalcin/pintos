...
/* Comparator: earlier wake_tick first. */
static bool
wake_tick_less (const struct list_elem *a,
                const struct list_elem *b,
                void *aux UNUSED)
{
  return list_entry (a, struct thread, sleep_elem)->wake_tick
       < list_entry (b, struct thread, sleep_elem)->wake_tick;
}
...
timer_sleep (int64_t ticks) 
{
  ...
  list_insert_ordered (&sleep_list, &cur->sleep_elem, wake_tick_less, NULL);
  thread_block ();
